...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_booking'])) {
    ...
    echo "<script>alert('Booking updated successfully!'); window.location='AP_recurring.php';</script>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['cancel_booking'])) {
    ...
    echo "<script>alert('Booking cancelled successfully!'); window.location='AP_recurring.php';</script>";
    exit;
}
...
// ✅ Fetch Recurring Bookings with Filter
if ($status_filter === 'All') {
    $sql = "SELECT * FROM bookings WHERE booking_type = 'Recurring' ORDER BY created_at DESC";
    $result = $conn->query($sql);
} else {
    $sql = "SELECT * FROM bookings WHERE booking_type = 'Recurring' AND status = ? ORDER BY created_at DESC";
...
<head>
    <meta charset="UTF-8">
    <title>Appointment Management -  Recurring Bookings</title>
...
<div class="status-tabs">
    <a href="AP_recurring.php?status=All" class="status-tab all <?= $status_filter === 'All' ? 'active' : '' ?>">
        <i class='bx bx-list-ul'></i> All
    </a>
    <a href="AP_recurring.php?status=Pending" class="status-tab pending <?= $status_filter === 'Pending' ? 'active' : '' ?>">
