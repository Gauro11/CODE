...
// ✅ Booking ID required
if (!isset($_GET['id'])) {
    echo "<script>alert('Invalid booking request.'); window.location.href='client_dashboard.php';</script>";
    exit;
}
...
if ($result->num_rows === 0) {
    echo "<script>alert('Booking not found or does not belong to you.'); window.location.href='client_dashboard.php';</script>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($_POST['rating_stars']) || !isset($_POST['rating_comment'])) {
        ...
        $materials = $_POST['materials_needed'] ?? NULL;
        ...
        // ✅ Save rating + comment + sentiment + materials
        $update = $conn->prepare("UPDATE bookings 
            SET rating_stars=?, rating_comment=?, materials_needed=?, sentiment=?, status='Completed'
            WHERE id=? AND email=?");

        $update->bind_param("isssis", $stars, $comment, $materials, $sentiment, $booking_id, $client_email);
        ...
        if ($update->execute()) {
            ...
            echo "<script>alert('Thank you! Your feedback has been submitted. Sentiment: {$sentiment}'); window.location.href='client_dashboard.php';</script>";
            exit;
        } else {
...
<head>
    <meta charset="UTF-8">
    <title>Rate Your Recurring Service</title>
    ...
    <style>
        ...
        textarea, input[type="text"] { 
            ...
            margin: 8px 0; min-height: 90px; box-sizing: border-box; 
        }
...
<div class="form-container">
    <h2 class="form-header">Rate Your Recurring Service</h2>
    ...
    <div class="job-details">
        ...
        <p><strong>Frequency:</strong> <?= htmlspecialchars($booking['frequency']) ?></p>
        <?php if (!empty($booking['preferred_day'])): ?>
        <p><strong>Preferred Day:</strong> <?= htmlspecialchars($booking['preferred_day']) ?></p>
        <?php endif; ?>
    </div>
    ...
    <form method="POST">
        ...
        <!-- <div class="rating-section">
            <h4>Materials Needed (optional)</h4>
            <input type="text" name="materials_needed" placeholder="Ex: mop, disinfectant, vacuum">
        </div> -->
